...
def plot3d(pts3, camL, camR):
    lookL = np.hstack((camL.t, camL.t + camL.R @ np.array([[0,0,4]]).T))
    lookR = np.hstack((camR.t, camR.t + camR.R @ np.array([[0,0,4]]).T))
...
def visualize_basic_mesh(pts3, tri, title):
    fig1 = plt.figure()
    ax = fig1.add_subplot(111, projection='3d')
    ax.plot_trisurf(pts3[0,:], pts3[1,:], pts3[2,:], triangles=tri,antialiased=False)
    set_axes_equal_3d(ax)
    ax.view_init(azim=180,elev=-90, vertical_axis='z')  #set the camera viewpointn
    label_axes(ax)
    plt.title(title)
    plt.show()
